...
noise_sd_values <- c(0.4, 0.5, 0.6)
diff_cols <- abs (vGRF_data_Phan("both")[,1]-vGRF_data_Phan("both")[,2])
  ...
  for (noise_fwhm in noise_fwhm_values) {
    ...
    while (sample_size <= max_sample_size 
      ...
      power_results <- Power_parallel(
        data = vGRF_data_Phan("both"), # Replace `your_data` with the actual data variable
...
write.xlsx(combined_results, "vGRF_nonconstant.xlsx")
